...
import { PrismaClient, type SupplierDocument } from "../generated/prisma/client.js";
...
export const generateDownloadLink = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (!file || file.supplierProfile?.userId !== userId) {
      return res.status(403).json({ message: "You do not have permission to access this file." });
    }

    const token = jwt.sign(
      { fileId: file.id, supplierProfileId: file.supplierProfileId },
...
export const downloadFile = async (req: AuthenticatedRequest, res: Response) => {
  const { fileUrl, fileName, fileType } = req.dbFile as { fileUrl: string; fileName: string; fileType: string };
